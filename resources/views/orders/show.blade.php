      ...
      <div class="lg:flex ">
        <div class="lg:w-1/3 p-4">
          <img
            ...
            alt="{{ $order->events?->judul ?? 'Event' }}" class="w-full object-cover mb-2" />
          <h2 class="font-semibold text-lg">{{ $order->events?->judul ?? 'Event' }}</h2>
          <p class="text-sm text-gray-500 mt-1">{{ $order->events?->lokasi ?? '' }}</p>
        </div>
